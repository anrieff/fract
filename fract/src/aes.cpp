...
/*
	Note: Some of the unnecesary source code is not included here.
*/
...
bool AESEncrypt(void *data, int octets)
{
	...
	// BEGIN ENCRYPTION
	keyInstance key;
	if (TRUE != makeKey(&key, DIR_ENCRYPT, 128, aeskey)) return false;
	cipherInstance cipher;
	if (TRUE != cipherInit(&cipher, MODE_ECB, NULL)) return false;

	unsigned char * temp = (unsigned char*) malloc(octets);
	if (!temp) return false;
	memcpy(temp, data, octets);
	blockEncrypt(&cipher, &key, (BYTE*)temp, octets * 8, (BYTE*)data);
	//
	free(temp);
	return true;
}
...
bool AESDecrypt(void *data, int octets)
{
	...
	// BEGIN DECRYPTION
	keyInstance key;
	if (TRUE != makeKey(&key, DIR_DECRYPT, 128, aeskey)) return false;
	cipherInstance cipher;
	if (TRUE != cipherInit(&cipher, MODE_ECB, NULL)) return false;
	...
	if (!temp) return false;
	memcpy(temp, data, octets);
	blockDecrypt(&cipher, &key, (BYTE*)temp, octets * 8, (BYTE*)data);
	//
	free(temp);
	return true;
}
