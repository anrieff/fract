...
int  ProjectSphere(sphere *a, const Vector& cur, const Vector w[3], int *dx, int *dy, int xres, int yres);
int  ProjectPoint(int *dx, int *dy, const Vector& d, const Vector& cur, Vector w[3], int xres, int yres);
int  ProjectPoint(float *x, float *y, const Vector& d, const Vector& cur, Vector w[3], int xres, int yres);
int  ProjectPointShadow(const Vector &vs, const Vector& l, int *x_2d, int *y_2d, int xres, int yres, Vector& cur, Vector& tt, Vector& a, Vector& b, bool & isfloor);
int  ProjectPointShadow(const Vector &vs, const Vector& l, float *coords, int xres, int yres, Vector &cur, Vector &tt, Vector &a, Vector &b, bool & isfloor, Vector& casted);
double normangle(double angle);
//int testsphere(sphere *a, double *v, double *cur);
Uint32 Solve3D(sphere *a, Vector& v, const Vector& c, const Vector& l, double Arcp, float *opacity, double *determinant, double *gB, int iteration, FilteringInfo &);
